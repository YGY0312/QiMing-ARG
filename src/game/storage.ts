...
function normalizeTab(tab: BrowserTabState, forcedId?: string, migratedAccountId?: string | null): BrowserTabState {
  ...
  return {
    ...
    pageTitle: siteType === 'student' && parseGameUrl(currentUrl).siteType === 'student' ? studentTabTitle(accountId) : parseGameUrl(currentUrl).pageTitle,
    refreshToken: 0,
...
export function migrateSave(value: unknown): GameSaveV5 | null {
  ...
  if (raw.schemaVersion === 5 || raw.schemaVersion === 4) {
    ...
    if (chapterDefaults.revealedFileSections.includes('chapter-six-final-read') && !triggeredEvents.includes('chapter_six_completed')) {
      triggeredEvents.push('chapter_six_completed')
    }
    ...
    if (chapterDefaults.revealedFileSections.includes('chapter-nine-final-read') && !triggeredEvents.includes('chapter_nine_completed')) {
      triggeredEvents.push('chapter_nine_final_opened', 'chapter_nine_completed')
    }
    return {
